...
class AccountCommonReport(models.TransientModel):
    ...
    def _build_contexts(self, data):
        ...
        res['branch_id'] = data['form']['branch_ids'] if data.get('form', False) and\
                             data.get('form', False).get('branch_ids', False) else False
        return res
...
class ReportJournal(models.AbstractModel):
    ...
    def _sum_debit(self, data, journal_id):
        ...
        query = 'SELECT SUM(debit) FROM ' + query_get_clause[0] + ', account_move am WHERE "account_move_line".move_id=am.id AND am.state IN %s AND "account_move_line".journal_id IN %s AND ' + query_get_clause[1] + ' '
        if data['form']['branch_ids']:
    ...
    def _sum_credit(self, data, journal_id):
        ...
        query_get_clause = self._get_query_get_clause(data)
        params = [tuple(move_state), tuple(journal_id.ids)] + query_get_clause[2]
        query = 'SELECT SUM(credit) FROM ' + query_get_clause[0] + ', account_move am WHERE "account_move_line".move_id=am.id AND am.state IN %s AND "account_move_line".journal_id IN %s AND ' + query_get_clause[1] + ' '
        if data['form']['branch_ids']:
            query += """AND account_move_line.branch_id IN (%s)""" % (','.join(map(str, data['form']['branch_ids'])))
        self.env.cr.execute(query, params)
        return self.env.cr.fetchone()[0] or 0.0
