...
class CompanyBranch(models.Model):
    ...
    def _name_search(self, name, args=None, operator='ilike', limit=100, name_get_uid=None):
        if self.env.context and self.env.context.get('multi_branch'):
            args += [('id', 'in', self.env.user.branch_ids.ids)]
        return super(CompanyBranch, self)._name_search(name, args=args, operator=operator, limit=limit,
...
class ResUsers(models.Model):
    ...
    def write(self, vals):
        ...
        if vals.get('company_id') and not vals.get('branch_id'):
            ...
            if branch_ids:
                ...
                raise ValidationError(_('There is no branch for company %s.' % (self.env['res.company'].browse(vals.get('company_id')).name)))
    ...
    branch_id = fields.Many2one('company.branch', string="Current Branch", default=lambda self: self._company_branch_unit())
